...
export const watchlistValuationsTable = pgTable("watchlist_valuations", {
  ...
  // AutoValue: Graham Number inputs/outputs, computed alongside AutoDCF in
  // the same watchlist run (see lib/valuation.ts#calculateGraham). Nullable
  // because eps/book value per share aren't always available from the
  // market data feed for every ticker.
  eps: real("eps"),
